...
float readChannel(ADS1115_MUX channel) {
  ...
  vc = adc.getResult_V(); // alternative: getResult_mV for Millivolt
  return vc;
}
...
public:
  ...
  float
    ...
    rawc[CELL_COUNT];                         // raw value to evaluate
...
public:
  ...
  void begin() {
   ...
   // pinMode(PORT_VOLT, INPUT);
    ...
    twizy.setInfoBmsType(0);
    twizy.setInfoState1(twizy.state()); 
    twizy.setInfoState2(bmsType_VirtualBMS); 
  ...
  void ticker(unsigned int clockCnt) {
    ...
    //
      ...
      if (!twizy.inState(Off)) {
        ...
        //curr_qa = ((analogRead(PORT_CURR) * VPORT) - OFFSET_VOLTAGE )/ V_per_A_QA;
        //curr_qa = ((analogRead(PORT_CURR)* VPORT) - OFFSET_VOLTAGE ) * 599.7 ;  // according to forumla Ip = (Vout - 1.833) * 596.242 * 4 ; *4 because of qa conversion
        
        curr_qa = (readChannel(ADS1115_COMP_1_GND) - OFFSET_VOLTAGE ) * 596.242 ; // use channel 1 of ADS1115; Vcc=5.029V!!
    ...
    if (!twizy.inState(Off) && clockCnt % 10 == 0) {  //original:10
      ...
      for (i=0; i < CELL_COUNT; i++) {
        ...
        // read stacked voltage from ADS1115, channel 0; Maybe add delay for ADC conv. time
        float vc = readChannel(ADS1115_COMP_0_GND) * SCALE_VOLT[i];
        rawc[i] = (readChannel(ADS1115_COMP_0_GND));
        // …smooth:
...
void loop() {
  ...
  twizy.looper();
  twizy.setError(TWIZY_OK); // original: nix
